...
- Usage: npm run make-admin -- <email-or-phone>
 */
...
async function run() {
  ...
  if (!identifier) {
    console.error('Usage: npm run make-admin -- <email-or-phone>');
    process.exit(1);
  }
  ...
  const user = await User.findOneAndUpdate(query, { $set: { role: 'admin' } }, { new: true });
  if (!user) {
  ...
  console.log(`Promoted ${user.email || user.phone} to admin.`);
  await mongoose.disconnect();
}
